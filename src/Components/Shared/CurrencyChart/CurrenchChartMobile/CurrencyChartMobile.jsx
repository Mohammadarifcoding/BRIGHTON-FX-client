...
const CurrencyChartMobile = ({data}) => {
    ...
    {
            item.map((item,ind) => <Tab  onClick={()=>{setActiveTab(ind)}} className={`bg-gray-300 w-[50%] text-center text-[#1E4A9A] border-none outline-none   sm:px-9 sm:py-4 px-5 py-3 `} key={item.id}>{item.name}</Tab>)
        }
    </TabList>
