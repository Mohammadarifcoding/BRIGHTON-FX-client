              ...
              <div className="mt-20 flex-1 gap-3 flex flex-col text-gray-900">
                ...
                {Data?.Orders?.map((item, index) => (
                  <h2 key={index} className="sm:text-xl mt-2">
                    Customer Give <span className="font-semibold">: {item?.currencyMy} {item?.currencyMycurrent}</span>
                    >> Customer get <span className="font-semibold">: {item?.currencyTake} {item?.currencyTakecurrent}</span> (Rate {(item?.Rate).toFixed(2)})
                  </h2>
                ))}
              </div>
            </div>
